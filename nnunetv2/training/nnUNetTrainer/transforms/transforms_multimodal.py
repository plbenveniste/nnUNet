...
class RedistributeTransformMultimodal(BasicTransform):
    ...
    def _apply_to_image(self, img: torch.Tensor, seg: torch.Tensor, **params) -> torch.Tensor:
        #for c in range(img.shape[0]): 
            # Here we take seg[0] instead of [c] because both images are associated to the same 
        img[0], seg[0] = aug_redistribute_seg(img[0], seg[0], classes=params['classes'], in_seg=params['in_seg'], retain_stats=params['retain_stats'])
        img[1], _ = aug_redistribute_seg(img[1], seg[0], classes=params['classes'], in_seg=params['in_seg'], retain_stats=params['retain_stats'])
        return img, seg
...
def aug_elastic(img, seg):
    subject = tio.RandomElasticDeformation(max_displacement=10)(tio.Subject(
        image=tio.ScalarImage(tensor=img),
        seg=tio.LabelMap(tensor=seg)
    ))
    img_out, seg_out = subject.image.data, subject.seg.data
